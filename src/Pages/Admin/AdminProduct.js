...
export const AdminProduct = () => {
    ...
    const handleDelete = async (id)=>{
        Swal.fire({
          ...
          if (result.isConfirmed) {
            try {
              axiosInstance.delete(`/event/deleteEvent/${id}`);
            } catch (err) {
              console.log(err);
            }
              Swal.fire({
    ...
    const displayProduct = product
        .filter((product)=>{
            if (searchTerm == ""){
                return product
            }else if(product.eventTitle.replaceAll(/\s/g,'').toLowerCase().includes(searchTerm.toLowerCase().replaceAll(/\s/g,''))){
                return product
            }
        })
        ...
        .map(product => {
        return(
            <div className="card_container" key={product.id}><br/>
                    <img src={product.image?product.image:null} className='imgstyle'></img>
                    <p className="productText">{product.eventTitle}</p>
                    <div className="col-container">
                        {/* <Link onClick={scrollToTop}><iconbutton><img src={eye} className='grow'/></iconbutton></Link>
                        <Link to={`/updateEvent`} state={{ product:product }} onClick={scrollToTop}><iconbutton><img src={pencel} className='grow'/></iconbutton></Link> */}
        ...
        <>
                ...
                <div className='APcenter-container'>
                    <div className='APcontainer-2'>
                        ...
                        <div className='col-secondcontainer'>
                            <h1 className="h1Text">Events</h1>
                            <p className='APwebtext'>manage and add new events here</p>
                        </div>
            ...
            <section className='APthirdsection'>
                <div className='APthirdcontainer'>
                    <input className='APsearchinput' type='text' placeholder='SEARCH EVENT...' onChange={event => {setSearchTerm(event.target.value)}}></input>
